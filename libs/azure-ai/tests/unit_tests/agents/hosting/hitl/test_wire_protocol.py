...
class TestMcpApprovalChannel:
    ...
    @REAL_INTERRUPT_ASYNC_XFAIL
    def test_reject_keeps_interrupt_pending(self, script: ScriptRegistrar) -> None:
        """A shortcut rejection fails without consuming the interrupt."""
        key = "hitl-reject"
        script(
            key,
            [
                    ...
                    tool_calls=[
                        {
                            "id": "call_ask_reject",
                            "name": "AskHuman",
                            "args": {"question": "Confirm the action?"},
                        }
                    ],
                ),
                AIMessage(content="Resumed after the rejected shortcut."),
            ],
        )
        host = ResponsesHostServer(build_ask_human_graph(key))
        ...
        with client_for(host) as client:
            ...
            assert payload["error"]["code"] == "interrupt_rejected"

            resumed = client.post(
                "/responses",
                json={
                    "conversation": {"id": conversation_id},
                    "input": [resume_item(call_id, "approved later")],
                },
            )

        assert resumed.json()["status"] == "completed"
        assert "resumed after" in assistant_text(resumed.json()).lower()
    ...
    def test_rich_channel_supports_mixed_middleware_decisions(
        ...
        with client_for(host) as client:
            ...
            interrupt_calls = sentinels(first.json())
            assert len(interrupt_calls) == 1
            interrupt_value = json.loads(interrupt_calls[0]["arguments"])["value"]
            assert [
                action["name"] for action in interrupt_value["action_requests"]
            ] == [
                "approved_tool",
                "rejected_tool",
            ]
            assert len(interrupt_value["review_configs"]) == 2
            approval_id = approval_requests(first.json())[0]["id"]
            call_id = interrupt_calls[0]["call_id"]
            rejected = client.post(
                "/responses",
                json={
                    "conversation": {"id": conversation_id},
        ...
        assert resumed.json()["status"] == "completed"
        assert "Mixed review completed" in assistant_text(resumed.json())
