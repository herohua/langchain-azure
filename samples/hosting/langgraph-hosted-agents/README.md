...
| 8 | [HITL](responses/08_hitl/) | Approval-style human-in-the-loop using `langgraph.types.interrupt`. Pending interrupts surface through paired shortcut and rich channels. |
...
| 11 | [HITL middleware](responses/11_hitl_middleware/) | `HumanInTheLoopMiddleware` with batched tool reviews. Approve, reject, edit, respond, and mixed decisions use the rich `function_call_output` channel. |
