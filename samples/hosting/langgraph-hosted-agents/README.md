...
| 8 | [HITL](responses/08_hitl/) | Approval-style human-in-the-loop using `langgraph.types.interrupt`. Pending interrupts surface as `mcp_approval_request`; matching `mcp_approval_response` objects resume the graph with `approve` and optional `reason`. |
